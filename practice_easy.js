...
function generateRandomQuestion() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    let question = {};
    let questionType = Math.floor(Math.random() * 2); // 0: 10進数→2進数, 1: 10進数→16進数 (0と1のみに変更)
  ...
  questions.forEach((question, index) => {
    ...
    if (selectedAnswers[index]) {
      ...
      if (selectedOption === question.answer) {
        ...
        questionDiv.style.border = "2px solid #4CAF50"; // 正解の枠線（緑）
      } else {
        resultText.textContent = ` 不正解。正解は ${question.answer} です。`;
        questionDiv.style.backgroundColor = "#ffcccc"; // 不正解の背景色
        questionDiv.style.border = "2px solid #f44336"; // 不正解の枠線（赤）
      }
    } else {
      resultText.textContent = ` 未回答`;
      questionDiv.style.backgroundColor = "#fff0f0"; // 未回答の背景色
      questionDiv.style.border = "2px solid #FF9800"; // 未回答の枠線（オレンジ）
    }
