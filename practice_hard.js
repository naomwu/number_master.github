...
function generateRandomQuestion() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    let question = {};
    let questionType = Math.floor(Math.random() * 6); // 0-5までランダムに変更
    ...
    if (questionType === 0) {
      ...
      question.type = "binary-to-hexadecimal";
      question.number = num.toString(2);
      question.answer = parseInt(question.number, 2).toString(16).toUpperCase();
      question.explanation = getBinaryToHexadecimalExplanation(question.number); // 途中式を追加
    } else {
      question.type = "hexadecimal-to-binary";
      question.number = num.toString(16).toUpperCase();
      question.answer = parseInt(question.number, 16).toString(2);
      question.explanation = getHexadecimalToBinaryExplanation(question.number); // 途中式を追加
    }
...
// 2進数→16進数の解説と途中式
function getBinaryToHexadecimalExplanation(binary) {
  let explanation = `${binary} を16進数に変換するには、2進数を4桁ごとに区切り、それを16進数に変換します。\n途中式:\n`;
  let binaryPadded = binary.padStart(Math.ceil(binary.length / 4) * 4, "0"); // 4桁で区切るために0埋め
  let hex = "";
  for (let i = 0; i < binaryPadded.length; i += 4) {
    let chunk = binaryPadded.substring(i, i + 4);
    hex += parseInt(chunk, 2).toString(16).toUpperCase();
    explanation += `${chunk} → ${parseInt(chunk, 2)
      .toString(16)
      .toUpperCase()}\n`;
  }
  explanation += `結果: ${hex.toUpperCase()}`;
  return explanation;
}

// 16進数→2進数の解説と途中式
function getHexadecimalToBinaryExplanation(hex) {
  let explanation = `${hex} を2進数に変換するには、各桁を2進数に変換します。\n途中式:\n`;
  let binary = "";
  let hexDigits = "0123456789ABCDEF";
  for (let i = 0; i < hex.length; i++) {
    let digitValue = hexDigits.indexOf(hex[i]);
    let binaryPart = digitValue.toString(2).padStart(4, "0");
    binary += binaryPart;
    explanation += `${hex[i]} → ${binaryPart}\n`;
  }
  explanation += `結果: ${binary}`;
  return explanation;
}
...
function getQuestionText(type) {
  ...
  if (type === "binary-to-hexadecimal") return "2進数→16進数";
  if (type === "hexadecimal-to-binary") return "16進数→2進数";
  return "";
}
